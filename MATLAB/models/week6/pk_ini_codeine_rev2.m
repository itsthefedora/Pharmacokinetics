...
VmaxC6Ga 	= VmaxC6G * 1e-9 * 60 * molarMassDrug;	% mg / hr
VmaxMora 	= VmaxMor * 1e-9 * 60 * molarMassDrug;	% mg / hr
VmaxNorCa 	= VmaxNorC * 1e-9 * 60 * molarMassDrug;	% mg / hr

%kRelC6G		= VmaxC6Ga / AmC6G;
%kRelMor 	= VmaxMor / AmMor;
%kRelNorC 	= VmaxNorC / AmNorC;
kRelC6G		= 0.6;
kRelMor		= 1 - (0.6 + 0.125);
kRelNorC	= 0.125;
kRelSum 	= kRelC6G + kRelMor + kRelNorC;

kEm 		= (1 - kErFraction) * kE;
kEC6G 		= (kRelC6G / kRelSum) * kEm;
kMetMor 	= (kRelMor / kRelSum) * kEm;
kENorC 		= (kRelNorC / kRelSum) * kEm;
...
x.from = 'body';
x.to = '';
x.linker = pk_linear_linker( kEC6G );
%x.linker = pk_mm_linker( AmC6G, VmaxC6Ga );
model.connections{ end + 1 } = x;
x = pk_default_connection( );
x.from = 'body';
x.to = '';
x.linker = pk_linear_linker( kENorC );
%x.linker = pk_mm_linker( AmC6G, VmaxC6Ga );
model.connections{ end + 1 } = x;
...
x.from = 'body';
x.to = 'morphine';
x.linker = pk_linear_linker( kMetMor );
